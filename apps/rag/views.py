...
from apps.questionnaires.models import UserQuestionnaireAnswer
...
def _build_user_profile(user, topic: str) -> dict:
    """Assemble user profile from UserPreferences + questionnaire answers."""
    try:
        prefs = user.userpreferences
        profile = {
            'current_skills': [],
            'goals': [],
            'level': prefs.target_role or '',
        ...
        profile = {'current_skills': [], 'goals': [], 'level': '', 'budget': None, 'weekly_hours': None}
...
def _save_roadmap_to_db(user, topic: str, roadmap_dict: dict) -> LearningPath:
    """Save generated roadmap into LearningPath + LearningPathCourse."""
...
class RAGRoadmapGenerateView(APIView):
    """POST /api/rag/generate-roadmap/ — Generate personalized roadmap via RAG."""
    ...
    def post(self, request):
        ...
        learning_path = (
            ...
            .prefetch_related('path_courses__course__platform', 'path_courses__course__tags')
            .first()
        )
